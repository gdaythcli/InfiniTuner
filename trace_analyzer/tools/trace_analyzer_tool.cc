...
DEFINE_bool(output_ml_features_windows, false,
            ...
            "key size variation, value size average, value size median, value size variation, "
            "mean, mode, median, 1st Quartiles, 3rd Quartiles, skewness, and kurtosis for KV pair accessed");
DEFINE_int32(output_ml_features_windows_size, 10,
            "set the window size for the output txt file");
...
Status TraceAnalyzer::EndProcessing() {
  ...
  if (FLAGS_output_ml_features_windows && ml_feature_windows_f_){
    std::map<std::pair<int, int>, std::string> res;
    for (int type = 0; type < kTaTypeNum; type++){
      if (!ta_[type].enabled) {
        continue;
      }
      for (auto& stat_it : ta_[type].stats) {
        ...
        for (const auto& [window_index, window_data] : time_windows) {
          ...
          std::string printout(buffer_);
          res[{type, window_index}] = printout;
          // printf("res[{%d, %d}] = %s;\n", type, window_index, printout.c_str());
          }
        }
      }
    }
    ...
    std::string result;  
    bool stop_processing = false;  // Flag to control when to stop
    int start_window_index = INT_MAX;
    for (const auto& entry : res) {
        start_window_index = std::min(start_window_index, entry.first.second);
    }
    // Starting from window_index 0, iterate until we reach a window with no data for any type
    int window_index = start_window_index;
    while (!stop_processing) {
        std::string time_window_str;  // StringStream to accumulate strings for this window
        bool has_data = false;  // Flag to check if any type has data for the current window
        // For each time window, iterate through types
        for (int type = 0; type < 9; type++) {
            auto it = res.find({type, window_index});  // Find the entry for the current type and window_index

            if (it != res.end()) {
                // If the entry exists, append the value (printout) for this type and window_index
                time_window_str += it->second;  // Append the data
                // printf("time_window_str: %s\n", time_window_str.c_str());
                has_data = true;  // Indicate that there is data for this window
            } else {
                // If the entry does not exist, append a placeholder "null"
                time_window_str += "0,0,0.000000,0,0.000000,0.000000,0,0.000000,0.000000,0.000000,0.000000,";
                // printf("time_window_str: %s\n", time_window_str.c_str());
            }
        }

        // If no data was found for the current window_index, stop processing
        if (!has_data) {
            stop_processing = true;  // Stop when no data is found for any type
        } else {
            // After finishing one window, add the accumulated string for the window plus a newline
            result += time_window_str + "\n";
            window_index++;  // Move to the next window
        }
    }
    // printf("result str: %s\n", result.c_str());
    s = ml_feature_windows_f_->Append(result);
    if (!s.ok()) {
      fprintf(stderr, "Write access count distribution file failed\n");
      return s;
    }
...
Status TraceAnalyzer::KeyStatsInsertion(const uint32_t& type,
  ...
  if (FLAGS_output_ml_features_windows) {
    ...
    trace_u.ts = (ts - time_series_start_) / 1000000;
    trace_u.cf_id = cf_id;
    int window_index = trace_u.ts / FLAGS_output_ml_features_windows_size;
