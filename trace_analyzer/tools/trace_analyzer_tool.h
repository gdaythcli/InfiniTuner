...
struct TraceStats {
  ...
  std::map<int, std::list<TraceUnit>> time_windows;
  ...
  // std::map<int, std::vector<uint64_t>> key_access_counts_windows;
...
class TraceAnalyzer : private TraceRecord::Handler,
 ...
 private:
  ...
  std::unique_ptr<ROCKSDB_NAMESPACE::WritableFile> ml_feature_windows_f_;
